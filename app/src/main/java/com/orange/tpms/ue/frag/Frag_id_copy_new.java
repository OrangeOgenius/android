 ...
 */
public class Frag_id_copy_new extends Frag_base {
    ...
    public int onInflateLayout() {
        return R.layout.frag_id_copy_new;
    }
...
//            tvContent.setText(idCopyFragBean.getmMyBean().getHex());
//        }
    }
    ...
    public void onKeyTrigger(){
        ...
        if(Rocket.isTopRocketStack(Frag_id_copy_new.class.getSimpleName())){
    ...
    public void onKeyScan(){
        Rocket.writeOuterLog("Frag_id_copy_new::onKeyScan");
        if(swwSelect.isShown()){
        ...
        if(Rocket.isTopRocketStack(Frag_id_copy_new.class.getSimpleName())){
            Rocket.writeOuterLog("Frag_id_copy_new::isTopRocketStack");
            if (swwSelect.getSelectType() == SensorWayWidget.SELECT_TYPE_SCAN) {
                Rocket.writeOuterLog("Frag_id_copy_new::onKeyScan->SelectType:SELECT_TYPE_SCAN");
                HardwareApp.getInstance().scan();
                lwLoading.show(getResources().getString(R.string.app_scaning));
            }else{
                Rocket.writeOuterLog("Frag_id_copy_new::onKeyScan->SelectType:"+swwSelect.getSelectType());
            }
        }
...
//        idCopyAdapter = new IDCopyAdapter(activity);
        ...
        dataReceiver = new HardwareApp.DataReceiver() {
            ...
            public void scanMsgReceive(String content) {
                Rocket.writeOuterLog("Frag_id_copy_new::scanMsgReceive->content:"+content);
                lwLoading.hide();
    ...
    private void initHelper() {
        ...
        copyIDHelper.setOnProgressListener(progress -> {
            ...
            String content = decimalFormat.format(progress);//format 返回的是字符串
